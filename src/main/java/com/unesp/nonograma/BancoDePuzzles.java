...
public class BancoDePuzzles {
    ...
    //FIca com o primeiro que resultar em solucao unica
    private static final int[] LIMIARES_CANDIDATOS = {100, 128, 150, 180};

    private final Map<CalculadoraDificuldade.Nivel, List<PuzzleGerado>> banco = new EnumMap<>(CalculadoraDificuldade.Nivel.class);
    ...
    private void processarImagem(File arquivo){
        ...
        try{
            ...
            System.err.println("Não foi possivel ler " + arquivo.getName() + ": " + e.getMessage());
            return;
        }
        ...
        for(int limiar : LIMIARES_CANDIDATOS){
            ...
            List<Tabuleiro.Estado[][]> todasSolucoes = SolverNonograma.gerarTodasSolucoes(pistasLinha, pistasColuna, LINHAS, COLUNAS);

            //So aceita se a solucao for unica - senao o puzzle é ambíguo
            ...
            System.out.printf("%s -> nível %s (slack médio %.2f, limiar %d%n", arquivo.getName(), nivel, slack, limiar);
            return;
        }

        System.err.println("Nenhum limiar gerou a solucao unica para " + arquivo.getName() + " - pulando.");
    }
    public PuzzleGerado sortear(CalculadoraDificuldade.Nivel nivel){
        ...
        if(lista == null || lista.isEmpty()){
            throw new IllegalStateException(
                    "Nenhum puzzle disponivel para o nivel " + nivel +
